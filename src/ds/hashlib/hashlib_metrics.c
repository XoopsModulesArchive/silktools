...
void dump_time(struct timeval *t) {
    fprintf(stderr, "tv_sec = %d\n", t->tv_sec);
    fprintf(stderr, "tv_usec = %d\n", t->tv_usec);
}

double get_elapsed_secs(struct timeval *start, struct timeval *end) {
...
/* Run a test */
HashTable *do_test(TestDesc *test_ptr) {
    uint32_t i;
    uint32_t key;
    uint32_t *val_ptr;
    HashTable *table_ptr;
...
double run_test(FILE *out_fp, TestDesc *test_ptr) {
    ...
    /* Print results */
    fprintf(out_fp, "%u\t%3.3f\t%llu\t%u\t%d\t%u\t%3.3f\t%llu\t%llu\t%llu\t%u\n",
            test_ptr->num_entries,
            test_ptr->estimate_ratio,
