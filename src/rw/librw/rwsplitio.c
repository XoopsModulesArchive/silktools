...
RCSIDENT("rwsplitio.c,v 1.8 2004/03/10 21:38:58 thomasm Exp");
...
rwIOStructPtr rwOpenSplit(rwIOStructPtr rwIOSPtr) {
  ...
  if (gHdr.version == 0) {
    ...
    rwIOSPtr->rwReadFn = rwReadSplitRec_V1;
  } else {
    fprintf(stderr, "Version mismatch\n");
    return _errorReturn(rwIOSPtr);
  }
  rwIOSPtr->rwSkipFn = rwSkipSplitRec_V1;
  ...
  /* 22 bytes on disk with 2 bytes padding in core */
  rwIOSPtr->recLen = 22;
...
uint32_t rwReadSplitRec_V1(rwIOStructPtr rwIOSPtr, rwRecPtr rwRP) {
  uint8_t ar[22]; /* 22 bytes on disk with 2 bytes padding in core */
...
uint32_t rwSkipSplitRec_V1(rwIOStructPtr rwIOSPtr, int skipCount) {
  ...
  for (i = 0; i < skipCount; i++) {
    if (! fread(&rwrec, sizeof(rwSplitRec_V1), 1, rwIOSPtr->FD)) {
      return 1;			/* failed */
    }
  }
  return 0;
