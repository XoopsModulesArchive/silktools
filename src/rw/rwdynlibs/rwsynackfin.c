...
static uint32_t finCount, ackCount, rstCount, synCount;
...
*/
int setup() {
  synCount = finCount =  ackCount =  rstCount = 0;
  return DYNLIB_WILLPROCESS;
}
...
int check(rwRecPtr rwRP) {
  ...
  if (rwRP->dPort != 80  && rwRP->dPort != 443) {
    return 1;
  }
  
  if (rwRP->flags & FIN_FLAG) {
    finCount++;
  }
  if (rwRP->flags == 16 && rwRP->pkts == 1 && rwRP->bytes == 40) {
    ackCount++;
  }
  if (rwRP->flags & RST_FLAG) {
    rstCount++;
  }
  if (rwRP->flags & SYN_FLAG) {
    synCount++;
  }
...
void teardown() {
  ...
  teardownFlag = 1;
  fprintf(stdout, "WEB SYN %u FIN %u RST %u ACK %u\n", synCount, finCount,
	  rstCount, ackCount);
  return;
}
